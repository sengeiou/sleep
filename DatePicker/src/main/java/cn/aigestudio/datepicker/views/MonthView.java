...
public class MonthView extends View {
    ...
    private void drawBG(Canvas canvas, Rect rect, DPInfo info) {
        ...
        if (!info.strG.equals("")&&chooseDate.equals(String.format("%04d-%02d-%02d",centerYear,centerMonth,Integer.valueOf(info.strG)))){
            drawBGChoose(canvas,rect);
        }
...
//            drawBGToday(canvas, rect);
    ...
    private void drawBGToday(Canvas canvas, Rect rect) {
        ...
        canvas.drawLine(0f, (float) rect.centerY(), (float) rect.centerX(),(float) rect.centerY(), mPaint);
//        canvas.drawCircle(rect.centerX(), rect.centerY(), circleRadius / 2F, mPaint);
    }

    private void drawBGChoose(Canvas canvas, Rect rect) {
        mPaint.setColor(mTManager.colorToday());
        canvas.drawCircle(rect.centerX(), rect.centerY(), circleRadius / 2F, mPaint);
    }
    ...
    void setDate(int year, int month,int day) {
        centerYear = year;
        centerMonth = month;
        chooseDate = String.format("%04d-%02d-%02d",year,month,day);
        indexYear = 0;
        indexMonth = 0;
        buildRegion();
        computeDate();
            ...
            for (int j = 0; j < tmp[i].length; j++) {
                ...
                if (region.contains(x, y)) {
                    ...
                    if (mDPMode == DPMode.SINGLE) {
                        if (touchAble){
                            touchAble = false;
                            chooseDate = String.format("%04d-%02d",centerYear,centerMonth)+mCManager.obtainDPInfo(centerYear, centerMonth)[i][j].strG;
                            cirApr.clear();
                            regions.add(region);
