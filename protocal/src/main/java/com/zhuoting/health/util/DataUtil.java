...
public class DataUtil {
	 ...
	 * 数据分包发送的情况下，拼接数据
	 * */
	static public byte[]ArrayListTobytes(ArrayList<ByteData> list){
		byte[] bytes = null;
		Collections.sort(list);
		for (ByteData data:list){
			byte sumData[];
			if (bytes==null){
				sumData = new byte[data.getData().length-6];
				System.arraycopy(data.getData(),5,sumData,0,data.getData().length-6);
			}else {
				sumData = new byte[data.getData().length-6+bytes.length];
				System.arraycopy(bytes,0,sumData,0,bytes.length);
				System.arraycopy(data.getData(),5,sumData,bytes.length,data.getData().length-6);
			}
			bytes = sumData;
		}
		return bytes;
...
//		return format1((float) ((step / 1000f) * 0.416));
	...
	public static int ftTocm(int ft,int in){
		float mIn = ft*12+in;
		int cm = (int) (in*2.54);
		return cm;
	}
